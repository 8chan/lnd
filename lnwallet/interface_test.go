...
func calcStaticFee(numHTLCs int) monautil.Amount {
	const (
		...
		htlcWeight   = 172
		feePerKw     = monautil.Amount(250/4) * 1000
	)
...
func loadTestCredits(miner *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	satoshiPerOutput, err := monautil.NewAmount(btcPerOutput)
	if err != nil {
	...
	expectedBalance += monautil.Amount(int64(satoshiPerOutput) * int64(numOutputs))
	addrs := make([]monautil.Address, 0, numOutputs)
...
func createTestWallet(tempTestDir string, miningNode *rpctest.Harness,
	...
	cfg := lnwallet.Config{
		...
		DefaultConstraints: channeldb.ChannelConstraints{
			DustLimit:        500,
			MaxPendingAmount: lnwire.NewMSatFromSatoshis(monautil.SatoshiPerBitcoin) * 100,
			ChanReserve:      100,
			MinHTLC:          400,
...
func testDualFundingReservationWorkflow(miner *rpctest.Harness,
	...
	fundingAmount, err := monautil.NewAmount(5)
	if err != nil {
...
func testFundingTransactionLockedOutputs(miner *rpctest.Harness,
	...
	fundingAmount, err := monautil.NewAmount(8)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	...
	amt, err := monautil.NewAmount(900)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
...
func testFundingCancellationNotEnoughFunds(miner *rpctest.Harness,
	...
	fundingAmount, err := monautil.NewAmount(44)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	req := &lnwallet.InitFundingReserveMsg{
...
func testReservationInitiatorBalanceBelowDustCancel(miner *rpctest.Harness,
	...
	const numBTC = 4
	fundingAmount, err := monautil.NewAmount(numBTC)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}

	feePerKw := lnwallet.SatPerKWeight(
		numBTC * numBTC * monautil.SatoshiPerBitcoin,
	)
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:       chainHash,
		NodeID:          bobPub,
...
func testSingleFunderReservationWorkflow(miner *rpctest.Harness,
	...
	// towards Bob's side.
	fundingAmt, err := monautil.NewAmount(4)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	pushAmt := lnwire.NewMSatFromSatoshis(monautil.SatoshiPerBitcoin)
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	if err != nil {
		t.Fatalf("unable to query fee estimator: %v", err)
	}
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	const numTxns = 5
	const outputAmt = monautil.SatoshiPerBitcoin
	...
	for _, txDetail := range txDetails {
		...
		if txOuts, ok := blockTxOuts[*txDetail.BlockHash][txDetail.Hash]; !ok {
			...
			var destinationAddresses []monautil.Address
...
func testPublishTransaction(r *rpctest.Harness,
	...
	pubkeyHash := monautil.Hash160(pubKey.PubKey.SerializeCompressed())
	keyAddr, err := monautil.NewAddressWitnessPubKeyHash(pubkeyHash,
	...
	txFromOutput := func(tx *wire.MsgTx, payToPubKey *btcec.PublicKey,
		txFee monautil.Amount) *wire.MsgTx {
		// Create a script to pay to.
		payToPubkeyHash := monautil.Hash160(payToPubKey.SerializeCompressed())
		payToKeyAddr, err := monautil.NewAddressWitnessPubKeyHash(payToPubkeyHash,
			&chaincfg.RegressionNetParams)
		if err != nil {
			t.Fatalf("unable to create addr: %v", err)
		}
	...
	newTx := func() *wire.MsgTx {
		...
		txFee := monautil.Amount(0.1 * monautil.SatoshiPerBitcoin)
	...
	// into the mempool.
	txFee := monautil.Amount(0.05 * monautil.SatoshiPerBitcoin)
...
func testSignOutputUsingTweaks(r *rpctest.Harness,
	...
	for i := 0; i < 2; i++ {
		...
		pubkeyHash := monautil.Hash160(tweakedKey.SerializeCompressed())
		keyAddr, err := monautil.NewAddressWitnessPubKeyHash(pubkeyHash,
			&chaincfg.RegressionNetParams)
		if err != nil {
			t.Fatalf("unable to create addr: %v", err)
		...
		vm, err := txscript.NewEngine(keyScript,
			...
			nil, int64(monautil.SatoshiPerBitcoin))
		if err != nil {
			t.Fatalf("unable to create engine: %v", err)
		}
		if err := vm.Execute(); err != nil {
...
func testChangeOutputSpendConfirmation(r *rpctest.Harness,
	...
	output = &wire.TxOut{
		Value:    monautil.SatoshiPerBitcent,
		PkScript: bobPkScript,
	}
	tx = sendCoins(t, r, alice, bob, output, txFeeRate)
	txHash = tx.TxHash()
