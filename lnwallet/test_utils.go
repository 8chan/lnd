...
func CreateTestChannels() (*LightningChannel, *LightningChannel, func(), error) {
	channelCapacity, err := monautil.NewAmount(10)
	if err != nil {
		return nil, nil, nil, err
	}

	channelBal := channelCapacity / 2
	aliceDustLimit := monautil.Amount(200)
	bobDustLimit := monautil.Amount(1300)
	...
	aliceCommit := channeldb.ChannelCommitment{
		...
		CommitFee:     commitFee,
		FeePerKw:      monautil.Amount(feePerKw),
...
func blockFromHex(blockHex string) (*monautil.Block, error) {
	bytes, err := hex.DecodeString(blockHex)
	if err != nil {
		return nil, err
	}
	return monautil.NewBlockFromBytes(bytes)
}

// txFromHex parses a full Bitcoin transaction from a hex encoded string.
func txFromHex(txHex string) (*monautil.Tx, error) {
	bytes, err := hex.DecodeString(txHex)
	if err != nil {
		return nil, err
	}
	return monautil.NewTxFromBytes(bytes)
}
...
func calcStaticFee(numHTLCs int) monautil.Amount {
	const (
		...
		htlcWeight   = 172
		feePerKw     = monautil.Amount(24/4) * 1000
	)
