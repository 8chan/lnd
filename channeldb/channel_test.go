...
func createTestChannelState(cdb *DB) (*OpenChannel, error) {
	...
	localCfg := ChannelConfig{
		ChannelConstraints: ChannelConstraints{
			DustLimit:        monautil.Amount(rand.Int63()),
			MaxPendingAmount: lnwire.MilliSatoshi(rand.Int63()),
			ChanReserve:      monautil.Amount(rand.Int63()),
	...
	return &OpenChannel{
		...
		IsPending:         true,
		IdentityPub:       pubKey,
		Capacity:          monautil.Amount(10000),
		...
		LocalCommitment: ChannelCommitment{
			...
			CommitFee:     monautil.Amount(rand.Int63()),
			FeePerKw:      monautil.Amount(5000),
			CommitTx:      testTx,
...
func TestOpenChannelPutGetDelete(t *testing.T) {
	...
	closeSummary := &ChannelCloseSummary{
		...
		SettledBalance:    monautil.Amount(500),
		TimeLockedBalance: monautil.Amount(10000),
		IsPending:         false,
