...
func (m *mockConstraints) ChannelBudget(chans []Channel,
	balance monautil.Amount) (monautil.Amount, uint32) {
...
func (m *mockConstraints) MinChanSize() monautil.Amount {
	return 1e7
}
func (m *mockConstraints) MaxChanSize() monautil.Amount {
	return 1e8
}
...
func (m *mockChanController) SpliceIn(chanPoint *wire.OutPoint,
	amt monautil.Amount) (*Channel, error) {
	return nil, nil
}
...
func setup(t *testing.T, initialChans []Channel) (*testContext, func()) {
	...
	var availableFunds monautil.Amount = 10 * monautil.SatoshiPerBitcoin
	...
	testCfg := Config{
		...
		Heuristic:      heuristic,
		ChanController: chanController,
		WalletBalance: func() (monautil.Amount, error) {
			ctx.Lock()
			defer ctx.Unlock()
...
func TestAgentChannelOpenSignal(t *testing.T) {
	...
	newChan := Channel{
		ChanID:   randChanID(),
		Capacity: monautil.SatoshiPerBitcoin,
	}
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * monautil.SatoshiPerBitcoin})
...
func TestAgentChannelCloseSignal(t *testing.T) {
	...
	initialChans := []Channel{
		...
		{
			ChanID:   randChanID(),
			Capacity: monautil.SatoshiPerBitcoin * 2,
		},
	}
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	testCtx.Lock()
	testCtx.walletBalance += monautil.SatoshiPerBitcoin * 5
	testCtx.Unlock()
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: numChans,
			amt:     5 * monautil.SatoshiPerBitcoin,
		},
	)
	...
	for i := 0; i < numChans; i++ {
		...
		case openChan := <-chanController.openChanSignals:
			if openChan.amt != monautil.SatoshiPerBitcoin {
				t.Fatalf("invalid chan amt: expected %v, got %v",
					monautil.SatoshiPerBitcoin, openChan.amt)
			}
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     monautil.SatoshiPerBitcoin,
		},
	)
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * monautil.SatoshiPerBitcoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, monautil.SatoshiPerBitcoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func respondWithScores(t *testing.T, testCtx *testContext,
	channelBudget monautil.Amount, existingChans, newChans int,
...
func checkChannelOpens(t *testing.T, testCtx *testContext,
	allocation monautil.Amount, numChans int) []NodeID {

	var nodes []NodeID

	// The agent should attempt to open channels, totaling what we expect.
	var totalAllocation monautil.Amount
	chanController := testCtx.chanController.(*mockChanController)
	for i := 0; i < numChans; i++ {
		select {
		case openChan := <-chanController.openChanSignals:
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	var channelBudget monautil.Amount = 5 * monautil.SatoshiPerBitcoin
	...
	expectedAllocation := testCtx.constraints.MaxChanSize() * monautil.Amount(numNewChannels)
	...
	channelBudget = monautil.SatoshiPerBitcoin * 3 / 2
