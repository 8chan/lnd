...
func (m *mockHeuristic) NodeScores(g ChannelGraph, chans []Channel,
	fundsAvailable monautil.Amount, nodes map[NodeID]struct{}) (
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	select {
	case constraints.moreChansResps <- moreChansResp{1, 5 * monautil.SatoshiPerBitcoin}:
	case <-time.After(time.Second * 10):
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	walletBalance := monautil.Amount(monautil.SatoshiPerBitcoin * 2)
	...
	walletBalanceMtx.Lock()
	walletBalance += monautil.SatoshiPerBitcoin * 5
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	const walletBalance = monautil.SatoshiPerBitcoin * 10
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	var walletBalanceMtx sync.Mutex
	walletBalance := monautil.Amount(monautil.SatoshiPerBitcoin * 6)
	...
	// heuristic.
	walletBalanceMtx.Lock()
	walletBalance += 0.4 * monautil.SatoshiPerBitcoin
	walletBalanceMtx.Unlock()
	...
	// channels.
	select {
	case constraints.moreChansResps <- moreChansResp{1, monautil.SatoshiPerBitcoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func TestAgentPendingOpenChannel(t *testing.T) {
	...
	// The wallet will start with 6 BTC available.
	const walletBalance = monautil.SatoshiPerBitcoin * 6
