...
func calcStaticFee(numHTLCs int) monautil.Amount {
	const (
		commitWeight = monautil.Amount(724)
		htlcWeight   = 172
		feePerKw     = monautil.Amount(50 * 1000 / 4)
	)
	return feePerKw * (commitWeight +
		monautil.Amount(htlcWeight*numHTLCs)) / 1000
}
...
func createPayReqs(node *lntest.HarnessNode, paymentAmt monautil.Amount,
...
func testOnchainFundRecovery(net *lntest.NetworkHarness, t *harnessTest) {
	...
	skipAndSend := func(nskip int) func(*lntest.HarnessNode) {
		return func(node *lntest.HarnessNode) {
			...
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			err = net.SendCoins(
				ctxt, monautil.SatoshiPerBitcoin, node,
			)
			if err != nil {
	...
	// path.
	restoreCheckBalance(2*monautil.SatoshiPerBitcoin, 2, 1, skipAndSend(9))

	// Check that using a recovery window of 9 does not find the two most
	// recent txns.
	restoreCheckBalance(2*monautil.SatoshiPerBitcoin, 2, 9, nil)
	...
	restoreCheckBalance(4*monautil.SatoshiPerBitcoin, 4, 10, skipAndSend(19))

	// Check that recovering with a recovery window of 19 fails to find the
	// most recent transactions.
	restoreCheckBalance(4*monautil.SatoshiPerBitcoin, 4, 19, nil)

	// Ensure that using a recovery window of 20 succeeds with all UTXOs
	// found and the final balance reflected.
	restoreCheckBalance(6*monautil.SatoshiPerBitcoin, 6, 20, nil)
}
...
func testBasicChannelFunding(net *lntest.NetworkHarness, t *harnessTest) {
	...
	pushAmt := monautil.Amount(100000)
...
func testUpdateChannelPolicy(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, monautil.SatoshiPerBitcoin, carol)
	if err != nil {
		t.Fatalf("unable to send coins to carol: %v", err)
	}
...
func testChannelBalance(net *lntest.NetworkHarness, t *harnessTest) {
	...
	checkChannelBalance := func(node lnrpc.LightningClient,
		...
		balance := monautil.Amount(response.Balance)
		if balance != amount {
...
func testChannelUnsettledBalance(net *lntest.NetworkHarness, t *harnessTest) {
	const chanAmt = monautil.Amount(1000000)
	ctxb := context.Background()
...
func testChannelForceClosure(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		chanAmt     = monautil.Amount(10e6)
		pushAmt     = monautil.Amount(5e6)
		paymentAmt  = 100000
		numInvoices = 6
	)
	...
	carolExpectedBalance := monautil.Amount(carolStartingBalance) + pushAmt
	if monautil.Amount(carolBalResp.ConfirmedBalance) < carolExpectedBalance {
		t.Fatalf("carol's balance is incorrect: expected %v got %v",
...
func testSphinxReplayPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	chanAmt := monautil.Amount(100000)
	...
	assertAmountSent := func(amt monautil.Amount) {
...
func testMultiHopPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = monautil.Amount(100000)
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, monautil.SatoshiPerBitcoin, dave)
	if err != nil {
...
func testMultiHopSendToRoute(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, monautil.SatoshiPerBitcoin, net.Bob)
	if err != nil {
...
func testSendToRouteErrorPropagation(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, monautil.SatoshiPerBitcoin, charlie)
	if err != nil {
...
func testInvoiceSubscriptions(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = monautil.Amount(500000)
...
func testMaxPendingChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	carolBalance := monautil.Amount(maxPendingChannels) * amount
			...
			for _, chanUpdate := range graphUpdate.ChannelUpdates {
				...
				if chanUpdate.Capacity != int64(chanAmt) {
					t.Fatalf("channel capacities mismatch:"+
						...
						monautil.Amount(chanUpdate.Capacity))
				}
				numChannelUpds++
			}
...
func testAsyncPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ := context.WithTimeout(ctxb, channelOpenTimeout)
	channelCapacity := monautil.Amount(paymentAmt * 2000)
	chanPoint := openChannelAndAssert(
		ctxt, t, net, net.Alice, net.Bob,
		lntest.OpenChannelParams{
			Amt: channelCapacity,
	...
	availableBalance := monautil.Amount(info.LocalBalance) - chanReserve
...
func testMultiHopHtlcLocalTimeout(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		dustHtlcAmt    = monautil.Amount(100)
		htlcAmt        = monautil.Amount(30000)
		finalCltvDelta = 40
	)
...
func testSwitchCircuitPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = monautil.Amount(1000000)
	const pushAmt = monautil.Amount(900000)
	var networkChans []*lnrpc.ChannelPoint
...
func testSendUpdateDisableChannel(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Give Eve some coins.
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, monautil.SatoshiPerBitcoin, eve)
	if err != nil {
...
func testSweepAllCoins(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, monautil.SatoshiPerBitcoin, ainz)
	if err != nil {
		t.Fatalf("unable to send coins to eve: %v", err)
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoinsNP2WKH(ctxt, monautil.SatoshiPerBitcoin, ainz)
	if err != nil {
		t.Fatalf("unable to send coins to eve: %v", err)
	}
...
func testChannelBackupUpdates(net *lntest.NetworkHarness, t *harnessTest) {
	...
	numChans := 2
	chanAmt := monautil.Amount(1000000)
...
func testChanRestoreScenario(t *harnessTest, net *lntest.NetworkHarness,
	...
	const (
		chanAmt = monautil.Amount(10000000)
		pushAmt = monautil.Amount(5000000)
	)
...
func TestLightningNetworkDaemon(t *testing.T) {
	...
	handlers := &rpcclient.NotificationHandlers{
		OnTxAccepted: func(hash *chainhash.Hash, amt monautil.Amount) {
