...
func createPayReqs(ctx context.Context, node *lntest.HarnessNode,
	paymentAmt monautil.Amount, numInvoices int) ([]string, [][]byte,
	[]*lnrpc.Invoice, error) {
...
func testOnchainFundRecovery(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// path.
	restoreCheckBalance(2*monautil.SatoshiPerBitcoin, 1, skipAndSend(9))

	// Check that using a recovery window of 9 does not find the two most
	// recent txns.
	restoreCheckBalance(2*monautil.SatoshiPerBitcoin, 9, nil)
	...
	restoreCheckBalance(4*monautil.SatoshiPerBitcoin, 10, skipAndSend(19))

	// Check that recovering with a recovery window of 19 fails to find the
	// most recent transactions.
	restoreCheckBalance(4*monautil.SatoshiPerBitcoin, 19, nil)

	// Ensure that using a recovery window of 20 succeeds.
	restoreCheckBalance(6*monautil.SatoshiPerBitcoin, 20, nil)
}
