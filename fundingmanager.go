...
const (
	...
	minChanFundingSize = monautil.Amount(20000)
	...
	// in the real world.
	maxBtcFundingAmount = monautil.Amount(1<<24) - 1
...
type fundingConfig struct {
	...
	NumRequiredConfs func(monautil.Amount, lnwire.MilliSatoshi) uint16
	...
	// contract breach.
	RequiredRemoteDelay func(monautil.Amount) uint16

	// RequiredRemoteChanReserve is a function closure that, given the
	// channel capacity and dust limit, will return an appropriate amount
	// for the remote peer's required channel reserve that is to be adhered
	// to at all times.
	RequiredRemoteChanReserve func(capacity, dustLimit monautil.Amount) monautil.Amount

	// RequiredRemoteMaxValue is a function closure that, given the channel
	// capacity, returns the amount of MilliSatoshis that our remote peer
	// can have in total outstanding HTLCs with us.
	RequiredRemoteMaxValue func(monautil.Amount) lnwire.MilliSatoshi

	// RequiredRemoteMaxHTLCs is a function closure that, given the channel
	// capacity, returns the number of maximum HTLCs the remote peer can
	// offer us.
	RequiredRemoteMaxHTLCs func(monautil.Amount) uint16
...
type pendingChannel struct {
	...
	localBalance  monautil.Amount
	remoteBalance monautil.Amount
}
...
func (f *fundingManager) handleFundingOpen(fmsg *fundingOpenMsg) {
	...
	if amt < f.cfg.MinChanSize {
		f.failFundingFlow(
			fmsg.peer, fmsg.msg.PendingChannelID,
			lnwallet.ErrChanTooSmall(amt, monautil.Amount(f.cfg.MinChanSize)),
		)
		return
	}
...
func (f *fundingManager) handleInitFundingMsg(msg *initFundingMsg) {
	...
	var ourDustLimit monautil.Amount
